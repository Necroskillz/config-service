...
const defaultContext: UseThemeProps = { setTheme: (_) => {}, themes: [], activeTheme: 'system' };
...
const Theme = ({
  ...
  const applyTheme = useCallback(() => {
    ...
    const handleAttribute = (attr: Attribute) => {
      if (attr === 'class') {
        ...
        if (resolvedTheme) d.classList.add(resolvedTheme);
      } else if (attr.startsWith('data-')) {
        if (resolvedTheme) {
          d.setAttribute(attr, resolvedTheme);
        } else {
  ...
  const setTheme = useCallback(
    ...
    (value: any) => {
      const newTheme = typeof value === 'function' ? value(themeState) : value;
  ...
  const providerValue = useMemo(
    ...
    [resolvedTheme, themeState, setTheme, forcedTheme, enableSystem, themes]
  );
