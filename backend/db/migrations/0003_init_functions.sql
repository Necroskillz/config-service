-- migrate:up
CREATE FUNCTION is_service_version_valid_in_changeset(sv service_versions, changeset_id bigint) RETURNS boolean AS $$
SELECT (
        (
...
CREATE FUNCTION is_feature_version_valid_in_changeset(fv feature_versions, changeset_id bigint) RETURNS boolean AS $$
SELECT (
        (
...
CREATE FUNCTION is_link_valid_in_changeset(
    fvsv feature_version_service_versions,
    changeset_id bigint
) RETURNS boolean AS $$
SELECT (
        (
...
CREATE FUNCTION is_key_valid_in_changeset(k keys, changeset_id bigint) RETURNS boolean AS $$
SELECT (
        (
...
CREATE FUNCTION is_variation_value_valid_in_changeset(vv variation_values, changeset_id bigint) RETURNS boolean AS $$
SELECT (
        (
