...
// @Param service query []string true "Service versions"
...
func (h *Handler) GetNextChangesets(c echo.Context) error {
	...
	err := echo.QueryParamsBinder(c).MustUint("after", &afterChangesetID).MustStrings("service", &serviceVersions).BindError()
	if err != nil {
...
// @Accept json
// @Produce json
// @Param changeset_id query uint true "Changeset ID"
// @Param service query []string true "Service versions"
// @Param mode query string true "Mode"
// @Param variation query object true "Variation"
...
func (h *Handler) GetConfiguration(c echo.Context) error {
	...
	var serviceVersions []string
	var mode string
	err := echo.QueryParamsBinder(c).Uint("changeset_id", &changesetID).MustStrings("service", &serviceVersions).String("mode", &mode).BindError()
	if err != nil {
		return ToHTTPError(err)
	}
